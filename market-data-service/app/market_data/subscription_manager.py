...
class SubscriptionManager:
    def __init__(self, clients: Iterable[MarketFeedClient], per_client_limit: int) -> None:
        ...
        for instrument in instruments:
            async with self._lock:
                ...
                if instrument.instrument_id in self._subscriptions:
                    results.append(SubscriptionResult(instrument, False, "Already subscribed"))
                    continue
                client = self.get_least_busy_client()
                if client is None:
                    results.append(SubscriptionResult(instrument, False, "No available client"))
                    continue
            ...
            try:
                ...
                results.append(SubscriptionResult(instrument, True, None))
            except Exception as e:
                results.append(SubscriptionResult(instrument, False, str(e)))
    ...
    async def unsubscribe(
        ...
        for instrument in instruments:
            async with self._lock:
                ...
                if client is None:
                    results.append(SubscriptionResult(instrument, False, "Not subscribed"))
                    continue
                # Reserve removal
                self._subscriptions.pop(instrument.instrument_id, None)
                self._client_subscriptions[client].discard(instrument.instrument_id)
